...
/// <summary>
/// Attaches explicit Core CX call targets for runtime call forms which are not
/// represented by a direct <see cref="ResolvedCallInfo"/>.
/// </summary>
internal static class CoreCxCallAnnotationPass
{
    ...
    private static void AnnotateCoreDirectCalls(ProgramNode program)
    {
        ...
        var roots = ExecutableAstTraversal.GetRoots(
            program,
            coreFunctions);
        foreach (var node in roots.SelectMany(
        ...
        TypeEnvironment environment)
    {
        ...
        if (call.Callee is MemberExpressionNode member
            && ExpressionNameFacts.GetQualifiedName(member.Target) is { } targetName
        ...
        TypeEnvironment environment)
    {
        ...
        {
            var concreteCall = new ResolvedCallInfo(
                concrete.Declaration,
                concrete.TypeArguments,
                IsInstance: true);
            call.Semantic.ResolvedCall = concreteCall;
            member.Semantic.ResolvedCall = concreteCall;
            return;
        }
        ...
        int argumentCount)
    {
        ...
        var normalizedReceiver = TypeRefFacts.StripPointersAndAliases(receiverType);
        var sourceReceiver = SourceGenericReceiver(
            program,
            normalizedReceiver);
        var result = functionCatalog.Instances
            .Where(instance =>
                string.Equals(
    ...
    {
        if (instance.Declaration.OwnerTypeNode?.ToTypeRef(parser) is not { } ownerType)
        {
            return null;
        }
    ...
    {
        if (TypeRefFacts.GetBaseName(normalizedReceiver) is not { } receiverName)
        {
            return null;
        }
    ...
    private static IReadOnlyList<TypeRef> ReceiverTypeArguments(
        ...
        FunctionNode method)
    {
        var normalized = TypeRefFacts.StripPointersAndAliases(receiverType);
        if (normalized is TypeRef.Named { Arguments.Count: > 0 } named)
        {
            return named.Arguments;
        }

        if (TypeRefFacts.GetBaseName(normalized) is not { } receiverName)
        {
            return [];
        }
    ...
    private static void RetargetNewlyResolvedGenericCalls(
        ...
        FunctionCatalog? functionCatalog)
    {
        if (functionCatalog is null || functionCatalog.Instances.Count == 0)
        {
            return;
        }
        ...
        TypeRefParser typeRefParser)
    {
        ...
        foreach (var binding in FunctionLocalBindingFacts.Enumerate(function.Body))
        {
            SetType(
                environment,
                binding.Name,
    ...
    private static IReadOnlyDictionary<string, TypeRef> FunctionTypeSubstitutions(
        FunctionNode function)
    {
        ...
        TypeEnvironment environment)
    {
        ...
        var method = interfaceNode.Methods.SingleOrDefault(candidate =>
            string.Equals(candidate.Name, member.MemberName, StringComparison.Ordinal)
    ...
    private static TypeRef? ResolveCoreExpressionType(
        ...
        TypeEnvironment environment)
    {
        if (typeResolver.ResolveTypeRef(expression, environment) is { } resolved)
        {
            return resolved;
        }

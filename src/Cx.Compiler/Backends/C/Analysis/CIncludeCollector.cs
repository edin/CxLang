...
{
    public static IReadOnlyList<CInclude> Collect(ProgramNode program)
    {
        var includes = program.Includes
            .Concat(CDeclarationUsageAnalyzer.GetDeclarationsToInclude(program)
        ...
        if (CNullUsageAnalyzer.UsesNull(program)

    ...
    CNameManglerOptions? nameManglerOptions = null)
{
    public CTranslationUnit Lower(ProgramNode program)
    {
        var backend = CBackendContext.Create(
            ...
            nameManglerOptions);
        return Lower(program, backend);
    }

    public CLoweringResult LowerForEmission(
        ProgramNode program,
        IReadOnlyList<string>? entryPoints)
    {
        var backend = CBackendContext.Create(
            ...
            program.TypeAdapters,
            nameManglerOptions);
        var loweredEntryPoints = entryPoints is { Count: > 0 }
            ? program.Functions
                .Where(function => function.OwnerTypeNode is null)
                .Where(function => entryPoints.Any(entryPoint =>
                    FunctionEntryPointFacts.Matches(function, entryPoint)))
                .Select(backend.NameMangler.FunctionName)
                .Distinct(StringComparer.Ordinal)
                .ToList()
            : null;
        return new CLoweringResult(
            Lower(program, backend),
            loweredEntryPoints);
    }

    private static CTranslationUnit Lower(
        ProgramNode program,
...
internal sealed record CLoweringResult(
    CTranslationUnit TranslationUnit,
    IReadOnlyList<string>? EntryPoints);

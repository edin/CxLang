...
public sealed partial class CEmitter
{
    private sealed class CStatementLoweringPipeline
    {
        ...
        public CStatementLoweringPipeline(ImportedNameLowerer nameLowerer)
        {
            ...
            _loopLowerer = new CLoopStatementLowerer(nameLowerer, this);
        ...
        public List<CStatementNode> LowerBlock(IEnumerable<StatementNode> statements, int rawIndentLevel) =>
            statements.Select(statement => Lower(statement, rawIndentLevel)).ToList();

        public CStatementNode Lower(StatementNode statement, int rawIndentLevel = 1) => statement switch
        {
            LetStatement let => ToCLocalDeclaration(let, _nameLowerer),
            ...
            IfStatement ifStatement => _conditionalLowerer.LowerIf(ifStatement, rawIndentLevel),
            WhileStatement whileStatement => _loopLowerer.LowerWhile(whileStatement, rawIndentLevel),
            ForStatement forStatement => _loopLowerer.LowerFor(forStatement, rawIndentLevel),
            ForeachStatement foreachStatement => throw UnexpectedUnloweredForeach(foreachStatement),
            SwitchStatement switchStatement => _switchLowerer.LowerSwitch(switchStatement, rawIndentLevel),
            MatchStatement matchStatement => throw UnexpectedUnloweredMatch(matchStatement),
            _ => ToRawCStatement(statement, _nameLowerer, rawIndentLevel),
        };
